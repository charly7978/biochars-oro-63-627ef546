...
export class HeartBeatProcessor {
  ...
  ARRHYTHMIA_THRESHOLD = 0.15; // Decreased to detect more arrhythmias
  CONSECUTIVE_IRREGULAR_BEATS = 3;
  arrhythmiaCounter = 0;
  consecutiveIrregularBeats = 0;
  lastArrhythmiaTime = 0;
  MIN_ARRHYTHMIA_INTERVAL_MS = 10000; // Reduced to allow more arrhythmia detections
  ...
  updateBPM() {
    ...
    if (instantBPM >= this.MIN_BPM && instantBPM <= this.MAX_BPM) {
      ...
      // Check for arrhythmia each time we update the BPM
      this.checkForArrhythmia();
    }
  }
  
  checkForArrhythmia() {
    // Need at least 3 intervals for arrhythmia detection
    if (this.bpmHistory.length < 3) return false;
    
    // Get the last interval
    const lastInterval = this.bpmHistory[this.bpmHistory.length - 1];
    
    // Calculate mean of previous intervals
    const prevIntervals = this.bpmHistory.slice(0, -1);
    const meanInterval = prevIntervals.reduce((sum, val) => sum + val, 0) / prevIntervals.length;
    
    // Calculate variation ratio
    const variationRatio = Math.abs(lastInterval - meanInterval) / meanInterval;
    
    // Check if variation exceeds threshold
    const isIrregular = variationRatio > this.ARRHYTHMIA_THRESHOLD;
    ...
    if (isIrregular) {
      ...
      // If we have enough consecutive irregular beats or a very high variation,
      // and enough time has passed since last arrhythmia
      if ((this.consecutiveIrregularBeats >= this.CONSECUTIVE_IRREGULAR_BEATS || 
          variationRatio > this.ARRHYTHMIA_THRESHOLD * 2) && 
          timeSinceLastArrhythmia > this.MIN_ARRHYTHMIA_INTERVAL_MS) {
        ...
        console.log(`HeartBeatProcessor: Arrhythmia detected! Count: ${this.arrhythmiaCounter}, Variation: ${variationRatio.toFixed(2)}`);
        
        return true;
      }
    } else {
      // Gradually reduce counter if beat is regular
      this.consecutiveIrregularBeats = Math.max(0, this.consecutiveIrregularBeats - 0.5);
    }
  ...
  // Method to get the arrhythmia counter
  getArrhythmiaCounter() {
    return this.arrhythmiaCounter || 0;
  }
  
  // Method needed by HeartRateProcessor
  setMonitoring(isMonitoring) {
    this.isMonitoring = !!isMonitoring;
    console.log(`HeartBeatProcessor: Monitoring set to ${isMonitoring}`);
  }
}
