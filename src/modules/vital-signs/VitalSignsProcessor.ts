...
export class VitalSignsProcessor {
  ...
  public processSignal(
    ...
    if ((signalVerified && hasPhysiologicalValidation) || timeSinceLastDetection < this.FALSE_POSITIVE_GUARD_PERIOD) {
      // Create a proper timestamp for the processor
      const now = Date.now();
      const result = this.processor.processSignal(ppgValue, rrData);
      
      // If we have arrhythmia data, make sure it's properly formatted
      if (result.arrhythmiaStatus && result.arrhythmiaStatus.includes("ARRHYTHMIA DETECTED") && 
          result.lastArrhythmiaData && result.lastArrhythmiaData.data) {
        // Format data correctly for downstream processing
        const formattedData = {
          timestamp: now,
          rmssd: result.lastArrhythmiaData.data.rmssd || 0,
          rrVariation: result.lastArrhythmiaData.data.rrVariation || 0
        };
