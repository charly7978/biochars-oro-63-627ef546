...
export function findPeaksAndValleys(values: number[]): { peakIndices: number[]; valleyIndices: number[] } {
  ...
  // Busca picos orientados hacia arriba (valores más altos)
  for (let i = 1; i < values.length - 1; i++) {
    const v = values[i];
    // Detección de picos (mayores valores = picos hacia arriba)
    if (
      v > values[i - 1] * 1.05 &&
      v > values[i + 1] * 1.05
    ) {
    ...
    // Detección de valles (menores valores = valles hacia abajo)
    if (
      v < values[i - 1] * 0.95 &&
      v < values[i + 1] * 0.95
    ) {
