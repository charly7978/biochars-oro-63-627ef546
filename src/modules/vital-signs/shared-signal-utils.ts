...
export class KalmanFilter {
  ...
  /**
   * Aplica filtro a un array
   */
  public filter(data: number[]): number[] {
    const result: number[] = [];
    for (let i = 0; i < data.length; i++) {
      result.push(this.update(data[i]));
    }
    return result;
  }
}
...
/**
 * Calcula componente AC de la señal PPG
 */
export function calculateAC(data: number[]): number {
  if (data.length === 0) return 0;
...
/**
 * Calcula componente DC de la señal PPG (valor medio)
 */
export function calculateDC(data: number[]): number {
  if (data.length === 0) return 0;
...
/**
 * Calcular la perfusión tisular a partir de datos PPG
 */
export function calculatePerfusionIndex(data: number[]): number {
  const ac = calculateAC(data);
  const dc = calculateDC(data);
