...
// Clase principal para encapsular los filtros
export class SignalProcessingFilters {
  /**
   ...
   */
  public static applySimpleMovingAverage(values: number[], windowSize: number = 5): number[] {
   ...
   */
  public static applyLowPassFilter(values: number[], alpha: number = 0.2): number[] {
   ...
   */
  public static removeLinearTrend(values: number[]): number[] {
   ...
   */
  public static normalizeValues(values: number[]): number[] {
    if (values.length === 0) return [];
   ...
   */
  public static detectSignalQuality(data: ProcessedPPGData): number {
   ...
   */
  public static detectFinger(rawValue: number, threshold: number = 0.2): boolean {
    // En un sistema real, usaríamos análisis de componentes, varianza, etc.
    return rawValue > threshold;
  }
}

// También exportamos las funciones individuales para mantener compatibilidad con código existente
export const {
