...
export function checkWeakSignal(
  ...
  if (isNaN(value)) {
    return { isWeakSignal: true, updatedWeakSignalsCount: currentWeakSignalsCount + 1 };
  }
...
export function updateLastValidBpm(result: any, lastValidBpmRef: React.MutableRefObject<number>): void {
  // Safety checks
  if (!result || !lastValidBpmRef) return;
...
export function processLowConfidenceResult(
  ...
  // Safety checks for invalid input
  if (!result) {
    return createWeakSignalResult(arrhythmiaCounter);
  }
  ...
  if (result.confidence < 0.6) { // Increased threshold for smoother transitions
    ...
    return {
      bpm: Math.round(newBpm) || 0,
      confidence: Math.max(0.15, result.confidence || 0),
