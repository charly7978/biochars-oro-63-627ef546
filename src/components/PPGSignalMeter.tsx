...
const PPGSignalMeter = ({ 
  ...
  useEffect(() => {
    if (rawArrhythmiaData && rawArrhythmiaData.timestamp > 0) {
      console.log("Updating peaks from arrhythmia data");
      ...
      // Check if the arrhythmia is already represented in peaks
      const arrhythmiaAlreadyExists = recentPeaks.some(
        peak => Math.abs(peak.time - rawArrhythmiaData.timestamp) < 500
      );
      
      // If not, add it
      if (!arrhythmiaAlreadyExists && arrhythmiaStatus?.includes("ARRITMIA")) {
        peaksRef.current.push({
          time: rawArrhythmiaData.timestamp,
  ...
  }, [rawArrhythmiaData, arrhythmiaStatus, WINDOW_WIDTH_MS]);
  ...
  const drawBackgroundGrid = useCallback((ctx: CanvasRenderingContext2D) => {
  ...
  return (
    <div className="fixed inset-0 bg-gradient-to-b from-white to-slate-50/30">
      <div className="absolute top-0 left-0 right-0 p-2 flex justify-between items-center backdrop-blur-sm border-b border-slate-100">
      ...
      <div className="fixed bottom-0 left-0 right-0 h-[60px] grid grid-cols-2 gap-px border-t border-slate-100">
        <button 
          onClick={onStartMeasurement}
          className="w-full h-full bg-transparent hover:bg-slate-50/80 text-xl font-bold text-slate-700 transition-all duration-300"
        >
          INICIAR
        </button>
        <button 
